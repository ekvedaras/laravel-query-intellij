...
class HasManyCall(override val reference: MethodReference, override val queryStatement: QueryStatement) : QueryMethodCall, DefinesModelRelation {
    override val columns: Set<RelationKeyParameter> = setOf(
        reference.getParameter(1).transformInstanceOf<StringLiteralExpression, RelationKeyParameter> {
            RelationKeyParameter(it.asStringParameter()) {
                queryStatement.queryVariable?.model
            }
        },
        reference.getParameter(2).transformInstanceOf<StringLiteralExpression, RelationKeyParameter> {
            RelationKeyParameter(it.asStringParameter()) {
